...
def create_BWT_matrix(string: str) -> pandas.core.frame.DataFrame:
  ...
  Returns:
    The Burrows-Wheeler Transform dataframe
  """
      
  s = string + '$'
  rotations = pd.DataFrame([list(s[i:] + s[:i]) for i in range(len(s))])
...
def suffix_array(string: str) -> list[int]:
  """Function to calculate suffix-array for a given string.
    ...
    Examples:
        ...
        [6, 5, 3, 1, 0, 4, 2]
    """
    rotations = create_BWT_matrix(string)
    sorted_rot = rotations.sort_values(by=0)
    sorted_ind = sorted_rot.index.to_list()
    return sorted_ind, sorted_rot
```
...
def BWT_from_suffix_array(
  ...
  """Function to calculate the Burrows-Wheeler Transform from a suffix array.
    ...
    Examples:
        ...
        'lo$oogg'
    """
    sorted_rot = suffix_array(text)[1]
    last_col = sorted_rot[len(text)-1]
