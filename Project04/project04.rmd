...
class DeBruijnGraph():
    ...
    def eulerian_walk(self, node, seed=None):
        ...
        #print(f"Available edges: {self.graph[node]}")
          
        tour = [node]
        edge = random.choice(self.graph[node])
        self.remove_edge(node, edge)
        node = edge
        
        if not self.graph[node]:
          #print(f"No more edges from '{node}', returning [{node}]")
          return tour
        else:
          next_tour = self.eulerian_walk(node)
          #print(f"Combining {tour} + {next_tour}")
          result = tour + next_tour
...
def eulerian_walk(self, node, seed=None):
        ...
        tour = [node]
        edge = random.choice(self.graph[node])  # randomly generate edge from node
        self.remove_edge(node, edge)
        
        next_tour = self.eulerian_walk(edge)  #using edge as the next node
        #print(f"Combining {tour} + {next_tour}")
        result = tour + next_tour
        #print(f"Returning: {result}")
        return result
