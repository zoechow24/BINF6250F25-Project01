...
class DeBruijnGraph():
    ...
    def build_debruijn_graph(self, input_string, k):
        ...
        for i in range(len(input_string)):
          #kmer <- input_string[i:k]  # ABCDEF
          left = input_string[i:i+k-1]  # ABCDE
          right = input_string[i+1:i+k]  # BCDEF
          if len(left) == k-1 and len(right) == k-1: #makes that kmers are proper length
    ...
    def print_eulerian_walk(self, seed=None):
        ...
        first_node = random.choice(list(self.graph.keys()))
        return self.eulerian_walk(first_node)
    ...
    def eulerian_walk(self, node, seed=None):
        ...
        while self.graph[node] != []:
          edge = random.choice(self.graph[node])      #list(self.graph.values()))  # randomly generate edge from node
          self.remove_edge(node, edge)
          node = edge  # update node 
          tour.append(edge)  # add edge to list
...
```{python}
def eulerian_walk2(self, node, seed=None):
        ''' This is a recursive function that follows all edges from a node
        to traverse the graph
        ...
        tour = list(node)  
        edge = random.choice(list(self.graph.values()))  # randomly generate edge from node
        tour.append(edge)  # add edge to list
        node = edge  # update node 
        remove_edge(node, edge)
        
        if debruijn_graph[node] == []:
          return tour
        else:
          next_tour = eulerian_walk(node)
          tour = tour + next_tour
          return tour
```

```{python}
debruijn_graph=defaultdict(list)  # initialize defaultdict
# | eval: false
